...
// Check we panic when given an Authenticate and UseDefaultAuth
func TestDualAuth(t *testing.T) {
	defer ensurePanic(t, "Added a route with Authenticate and UseDefaultAuth options set")
	api := getTestApi()
	api.AddDefaultRoutes(&Widget{}, RouteOptions{Authenticate: func(req ReqToAuthenticate) bool { return true }, UseDefaultAuth: true})
}

// Add CheckLoginDetails so User implements LoginModel
func (_ *User) CheckLoginDetails(json *map[string]interface{}, g *Grapi) (uint, error) {
	user := User{}
...
func (_ *User) GetById(id uint, g *Grapi) (LoginModel, error) {
	user := User{}
...
func TestAuthenticatication(t *testing.T) {
	...
	testReq(t, "Login(Malformed JSON)", "POST", "/api/auth", `{"name: "admin", "password": "password"}`, 422)
	testReq(t, "Login(Wrong password)", "POST", "/api/auth", `{"name": "admin", "password": "wrongpassword"}`, 403)
	token := getToken(testReq(t, "Login", "POST", "/api/auth", `{"name": "admin", "password": "password"}`, 200))
